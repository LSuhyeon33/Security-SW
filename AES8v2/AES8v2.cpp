// AES8v2.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
#include <iostream>
#include <time.h>
...
void AES8_Round(byte state[16], byte rk[16]) {
	AES8v2_SubBytes(state);
	AES8v2_ShiftRows(state);
	AES8v2_MixColumn(state);
	AES8v2_AddRoundkey(state, rk);
}
...
void AES8_Encrypt(byte PT[16], byte RK[11][16], byte CT[16]) {
	...
	AES8v2_SubBytes(state);
	AES8v2_ShiftRows(state);
	AES8v2_AddRoundkey(state, RK[10]);
